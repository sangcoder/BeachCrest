  ...
  <div class="history-booking">
    <div class="card">
      <div class="card-body">
        ...
        >
          <template slot="tour" slot-scope="tour">
            <a
              target="_blank"
              href="javascript:;"
              rel="noopener noreferrer"
              @click="$router.push({name: 'booking', query: {tour: tour.infoTour.id}})"
            >{{tour.infoTour.name | truncate(45)}}</a>
          </template>
          <template
            slot="State"
            slot-scope="State"
          ...
          >{{totalAmount && totalAmount.payment && totalAmount.payment.PaymentAmount | toCurrency}}</template>
          <template
            slot="type"
            slot-scope="type"
          >{{type && type.payment && type.payment.PaymentType === 1 ? 'Normal' : 'Paypal'}}</template>
        </a-table>
      </div>
    </div>
  </div>
